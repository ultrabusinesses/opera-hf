...
uint256 hashGenesisBlock("0x00000509bf6102f3cef63221a3b0b4da373cf0d1b754086e173bf5c119a119f1");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // OperaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "OperaCoin Signed Message:\n";
...
{
    // OperaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 1 * 120; // OperaCoin: each block
static const int64 nTargetSpacing = 1 * 120; // OperaCoin: 120 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // OperaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, OperaCoin - DarkGravity, written by Evan Duffield - evan@OperaCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // OperaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // OperaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        assert(block.hashMerkleRoot == uint256("0x859952ef6f6ebec28e09d00f5661bb174fb9cd91c1c780bffd4b1d6760ad9333"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // OperaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OperaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OperaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OperaCoinMiner(CWallet *pwallet)
{
    printf("OperaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("OperaCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running OperaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OperaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OperaCoinMiner, pwallet));
}
